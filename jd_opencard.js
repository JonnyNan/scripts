...
async function run(){
    try{
        ...
        await $.wait(1000)
        $.log("关注: " + $.followShop)
        await saveTask('关注', 23, 1000282702);
        await $.wait(1000)
        $.log("加购: " + $.addSku)
        await saveTask('加购', 2, 100021564598);
        await $.wait(1000)
        await getActorUuid()
        await $.wait(1000)
...
function saveTask(title, taskType, taskValue) {
    let url = '/dingzhi/dz/openCard/saveTask'
    if(title == "关注"){
        url = '/dingzhi/dz/openCard/followShop'
    }else if(title == "加购"){
        url = '/dingzhi/dz/openCard/saveTask'
    }
    return new Promise(resolve => {
...
function shopactivityId(functionId) {
    return {
        ...
        headers: {
            ...
            'Referer': `https://shopmember.m.jd.com/shopcard/?venderId=${functionId}&shopId=${functionId}&venderType=5&channel=401&returnUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
            'Cookie': cookie
        }
    }
}
            ...
            try {
                if (err) {
                    ...
                    if(typeof res == 'object' && res.result === true){
                        if(typeof res.data.followShop != 'undefined') $.followShop = res.data.followShop
                        if(typeof res.data.addSku != 'undefined') $.addSku = res.data.addSku
                        if(typeof res.data.actorUuid != 'undefined') $.actorUuid = res.data.actorUuid
                        if(typeof res.data.scoreList != 'undefined') $.scoreList = res.data.scoreList
...
function accessLogWithAD() {
    return new Promise(resolve => {
        let pageurl = `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
...
function getToken() {
    return new Promise(resolve => {
        $.post({
            ...
            body: 'area=19_1601_50258_50374&body=%7B%22url%22%3A%22https%3A%5C/%5C/lzdz1-isv.isvjcloud.com%22%2C%22id%22%3A%22%22%7D&build=167798&client=apple&clientVersion=10.1.2&d_brand=apple&d_model=iPhone13%2C2&eid=eidI2ad5812337s3ghGuVeflROmNL7t9SyzhxIIRr8y39ehVQQzh3oXWc/QdZtuYpoU84EZvJqBkJ%2BWEUsc7iuc80hh3Y5nnWIpS4d3eVHSbXWGspkRV&isBackground=N&joycious=119&lang=zh_CN&networkType=4g&networklibtype=JDNetworkBaseAF&openudid=753d213009c85f60f8ce9df3a678389ffa3fb1c5&osVersion=14.7.1&partner=TF&rfs=0000&scope=11&screen=1170%2A2532&sign=61cda8ac0a6a73c6bbf6ac96201e22c4&st=1630899839541&sv=112&uemps=0-0&uts=0f31TVRjBStKxSdC8yu2b54wFO8pbDscKEz/Lzda0TwHKy9aum3JkFuJto5oAw6BebqjSqPeCCyhG4enEi%2BREXvGDRl/QSlPxxHj4/lkVIXyH9fs5fV/tAvjl4Y91p4lHzvOMSDp6ffI8c1urNpEKX69fs5%2BdwovkKxsEyPfSRj4TXIaeTpg1VFSNIJ7M8FbbdCrOTcwbyZZNQn4hbtH1Q%3D%3D&uuid=hjudwgohxzVu96krv/T6Hg%3D%3D&wifiBssid=unknown',
            headers: {
                'Content-Type':'application/x-www-form-urlencoded',
                'Cookie': cookie,
                'Host':'api.m.jd.com',
