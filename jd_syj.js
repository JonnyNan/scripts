...
!(async () => {
  ...
  await getAuthorShareCode('https://cdn.jsdelivr.net/gh/1277002811/JDbot@master/shareCodes/jd_zz.json');
...
async function getRandomCode() {
  await $.http.get({url: `https://cdn.jsdelivr.net/gh/1277002811/JDbot@master/jd_zz.json`, timeout: 10000}).then(async (resp) => {
    if (resp.statusCode === 200) {
      try {
        let { body } = resp;
        body = JSON.parse(body);
