...
第11个之后是不会有京豆的
入口
https://lzdz1-isv.isvjcloud.com/dingzhi/personal/care/activity/1931106?activityId=fb64cd8222a74c0f96557c2d97547e76&shareUuid=a2af81c7f1604724b24f2e0e72bfa744
...
!(async () => {
  ...
  $.shareUuid = ['06e38ac5a09e48ffa122c70b50ea347c']
...
function openCardStartDraw(type) {
  return new Promise(resolve => {
    let body = `activityId=fb64cd8222a74c0f96557c2d97547e76&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&type=${type}`
...
function getDrawRecordHasCoupon() {
  return new Promise(resolve => {
    let body = `activityId=fb64cd8222a74c0f96557c2d97547e76&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}&num=0&sortSuatus=1`
...
function startDraw() {
  return new Promise(resolve => {
    let body = `pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=fb64cd8222a74c0f96557c2d97547e76&actorUuid=${$.actorUuid}&change=3`
    $.post(taskPostUrl('/dingzhi/dz/openCard/startDraw', body), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function followShop() {
  return new Promise(resolve => {
    let body = `pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=fb64cd8222a74c0f96557c2d97547e76&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}&taskType=23&taskValue=1000003443`
...
function shopactivityId(functionId) {
  return {
    ...
    headers: {
      ...
      'Referer': `https://shopmember.m.jd.com/shopcard/?venderId=${functionId}&shopId=${functionId}&venderType=5&channel=401&returnUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/personal/care/activity/1931106?activityId=fb64cd8222a74c0f96557c2d97547e76&shareUuid=${$.shareUuid}`,
      'Cookie': cookie
    }
  }
}
...
function getHtml() {
  ...
  return new Promise(resolve => {
    $.get({
      url: `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/1931106?activityId=fb64cd8222a74c0f96557c2d97547e76&shareUuid=${$.shareUuid}`,
      headers: {
        'User-Agent': $.UA,
        'Host':'lzdz1-isv.isvjcloud.com',
...
function drawContent() {
  return new Promise(resolve => {
    $.post({
      ...
      body: `activityId=fb64cd8222a74c0f96557c2d97547e76&pin=${encodeURIComponent($.myPingData.secretPin)}`,
      headers: {
        'User-Agent': $.UA,
        'Host':'lzdz1-isv.isvjcloud.com',
        'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
        'Referer':`https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/1931106?activityId=fb64cd8222a74c0f96557c2d97547e76&shareUuid=${$.shareUuid}`,
        'Cookie': `LZ_TOKEN_KEY=${$.LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${$.LZ_TOKEN_VALUE}; AUTH_C_USER=${$.myPingData.secretPin}; ${$.lz_jdpin_token}`,
      }
    }, async (err, resp, data) => {
      try {
...
function getActorUuid() {
  return new Promise(resolve => {
    $.post({
      ...
      body: `activityId=fb64cd8222a74c0f96557c2d97547e76&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=${encodeURIComponent($.attrTouXiang)}&nick=${encodeURIComponent($.myPingData.nickname)}&shareUuid=${$.shareUuid}`,
      headers: {
        'User-Agent': $.UA,
        'Host':'lzdz1-isv.isvjcloud.com',
        'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
...
function taskPostUrl(url, body) {
  return {
    ...
    headers: {
      ...
      "Referer": `https://lzdz1-isv.isvjcloud.com/dingzhi/personal/care/activity/1931106?activityId=fb64cd8222a74c0f96557c2d97547e76&shareUuid=${$.shareUuid}`,
      "User-Agent": $.UA ,
