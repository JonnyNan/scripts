...
!(async () => {
  ...
  await requireConfig()
  $.authorCode = await getAuthorShareCode('https://xr2021.coding.net/p/import-kasd/d/JDbot/git/raw/master/shareCodes/JD_Cash.json')
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://xr2021.coding.net/p/import-kasd/d/JDbot/git/raw/master/shareCodes/JD_Cash.json`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
