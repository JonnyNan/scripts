...
!(async() => {
	...
	if (intPerSent > 0) {
		...
		if ($.isNode() && allMessageMonthGp1) {
			await notify.sendNotify(`京东月资产变动#1`, `${allMessageMonthGp1}`, {
				url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean`
			})
		}
...
async function showMsg() {
	...
	if (llShowMonth) {
		...
		if (userIndex1 != -1) {
			allMessageMonthGp1 += ReturnMessageMonth + `\n`;
		}
		if (userIndex2 != -1) {
	...
	return new Promise(async resolve => {
		const options = {
			...
			"headers": {
				...
				'Connection': 'keep-alive',
				'Accept-Language': 'zh-cn',
				'Referer': 'https://st.jingxi.com/my/redpacket.shtml?newPg=App&jxsid=16341020658376231734',
				'Accept-Encoding': 'gzip, deflate, br',
				"Cookie": cookie,
				'User-Agent': $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.4;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1")
			}
				...
				if (err) {
					...
					if (data) {
						...
						let t = new Date()
							...
							$.message += `【总红包】${$.balance}(将过期${$.expiredBalance})元 \n`;
						if ($.jxRed > 0)
